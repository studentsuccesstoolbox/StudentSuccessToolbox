...
.factory('RzSlider', ['$timeout', '$document', '$window', 'throttle', function($timeout, $document, $window, throttle)
{
  ...
  Slider.prototype = {
    ...
    bindEvents: function()
    {
      ...
      //Addes keyboard support
      this.minH.on('keydown', angular.bind(this, this.keySliderDown, this.selBar));
      //this.minH.on('keydown', function(){alert('test')});
    ...
    keySliderDown: function(pointer, event){
        
       var newValue,newOffset;
       //Left
       if (event.keyCode == '37') {
           newValue = ((this.scope.rzSliderModel-1) <= this.minValue)?this.minValue:(this.scope.rzSliderModel-1);
           this.scope.rzSliderModel = newValue;
           this.scope.$apply();
           this.callOnChange();
           this.resetSlider();
       ...
       //Right
            ...
            newValue = ((this.scope.rzSliderModel+1) >= this.maxValue)?this.maxValue:(this.scope.rzSliderModel+1);
            this.scope.rzSliderModel = newValue;
            this.scope.$apply();
            this.callOnChange();
            this.resetSlider();
